...
def load_kpi_data(uploaded_file):
    ...
    try:
        df = pd.read_excel(uploaded_file, header=3) if uploaded_file.name.endswith('.xlsx') else pd.read_csv(
            uploaded_file, header=3)
...
# ==============================================================================
# HELPER FUNCTIONS (*** GAUGE FUNCTION UPDATED ***)
# ==============================================================================
def plot_kpi_gauge(kpi_data):
    ...
    # --- NEW: Create a text box annotation for P25, Median, P75 ---
    annotation_text = (
        f"<b>P25:</b> {p25:.2f}<br>"
    ...
    fig.add_annotation(
        x=0.05,  # Position horizontally (5% from the left)
        y=0.95,  # Position vertically (95% from the bottom)
        xref="paper",
        yref="paper",
...
def interpret_kpi_data(kpi_data):
    kpi_name, value, p25, median, p75 = kpi_data['kpi_name'], kpi_data['KPI Value'], kpi_data['P25'], kpi_data[
        'Median'], kpi_data['P75']
...
with st.sidebar:
    ...
    uploaded_file = st.file_uploader("เลือกไฟล์ XLSX หรือ CSV", type=['xlsx'])

if uploaded_file is not None:
    df_kpi = load_kpi_data(uploaded_file)
    if not df_kpi.empty:
    ...
    st.info("รอการอัปโหลดไฟล์ข้อมูล...")
